...
/*****************************************************************************/
/* History:
 * 02Mar16 gwb Changed mdm_responses from unsigned to const in order to
 *             eliminate compiler warnings.
 */

const char *mdm_responses[37];
...
int mdm_init()
{
    ...
    mdm_responses[MDM_RESP_INFO] = "TWR FAUX IRIDIUM 9522";
    mdm_responses[MDM_RESP_RSSI] = "+CSQ: 5";
    return 0;
}
...
void mdm_respond(modem_config *cfg, const char *msg)
{
    int len = strlen(msg);

    mdm_write(cfg, (char *)msg, len);
}
...
void mdm_send_response(int msg, modem_config *cfg)
{
    ...
    if (cfg->send_responses == TRUE)
    {
        ...
        if (cfg->text_responses == TRUE)
        {
            ...
            mdm_write(cfg, (char *)mdm_responses[msg], strlen(mdm_responses[msg]));
        }
        else
        {
...
int mdm_disconnect_quiet(modem_config *cfg, int quiet)
{
    int type;
...
int mdm_parse_cmd(modem_config *cfg)
{
    ...
    while (TRUE != done)
    {
        ...
        case 'I': // Information.
            mdm_send_response(MDM_RESP_INFO, cfg);
            break;
        ...
        case AT_CMD_FLAG_PRO_PLUS + '+':

            if (strcmd(command + index, "CREG?"))
            {
                mdm_respond(cfg, "\r\n+CREG:0,1\r\n");
            }
            else if (strcmd(command + index, "CBST=")
                     || strcmd(command + index, "IPR="))
            {
                sleep(4);
                // Nothing, just say OK
            }
            else if (strcmd(command + index, "CSQ"))
            {
                sleep(1);
                mdm_respond(cfg, "\r\n+CSQ:5\r\n");
            }
