...
/*****************************************************************************/
/* History:
 * 02Mar16 gwb Cleaned up compiler warnings generated by -Wpointer-sign
 */
        ...
        while (i < len)
        {
            ...
            if (NVT_IAC == ch)
            {
                ...
                case NVT_SB: // sub negotiation
                    // again, overflow...
                    i += parse_nvt_subcommand(cfg->line_data.fd, &cfg->line_data.nvt_data, data + i,
                                              len - i);
                    break;
...
void *ctrl_thread(void *arg)
{
    ...
    while (status > -1)
    {
        ...
        {
            LOG(LOG_INFO, "modem status change %x", new_status);
    ...
    for (;;)
    {
        if (last_conn_type != cfg->conn_type)
        {
            ...
            if (cfg->conn_type == MDM_CONN_OUTGOING)
            {
                ...
                {
                    writeFile((char *)cfg->data.local_connect, cfg->line_data.fd);
                }

                if (strlen((char *)cfg->data.remote_connect) > 0)
                {
                    writeFile((char *)cfg->data.remote_connect, cfg->line_data.fd);
                }
            }
            else if (cfg->conn_type == MDM_CONN_INCOMING)
            {
                ...
                {
                    writeFile((char *)cfg->data.local_answer, cfg->line_data.fd);
                }

                if (strlen((char *)cfg->data.remote_answer) > 0)
                {
                    writeFile((char *)cfg->data.remote_answer, cfg->line_data.fd);
                }
            }
        ...
        else if (rc == 0)
        {
                ...
                && (cfg->line_data.valid_conn == TRUE))
            {
                if ((cfg->s[0] == 0) && (cfg->rings == 10))
                {
                    ...
                    {
                        line_write(cfg, (char *)MDM_NO_ANSWER, strlen((char *)MDM_NO_ANSWER));
                    }
                    else
                    {
        ...
        if (FD_ISSET(cfg->dce_data.fd, &readfs)) // serial port
        {
            ...
            if (res > 0)
            {
                ...
                else
                {
                    ...
                    if (dce_get_control_lines(cfg) == 1)
                    {
                        mdm_disconnect(cfg);
                    }
                }
