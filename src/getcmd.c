...
/*****************************************************************************/
/* History:
 * 02Mar16 gwb Made changes to eliminate warnings caused by -Wpointer-sign
 *
 */
...
int strcmd(const char *line, const char *cmd)
{
    ...
    return strnicmp(line, cmd, cmdlen) == 0;
}
    ...
    while (*index < len)
    {
        ...
        case '&':
            ...
            while (*index < len)
            {
                ...
                case 'Z':
                    return parseRegister(line, AT_CMD_FLAG_EXT, index, num, len, data_start,
                                         data_end, TRUE);
        ...
        case 'S':
            return parseRegister(line, AT_CMD_FLAG_BAS, index, num, len, data_start, data_end,
                                 FALSE);
