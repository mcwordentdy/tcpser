...
/* History:
 *
 * 20Jan14 gwb Expanded "-p" command line option to allow tcpser to bind to a specific
 *             IP address as well as a specific port.
 *
 */
            ...
            if (!accept_pending)
            {
                ...
                if (i == modem_count)
                {
                    ...
                    if (cSocket > -1)
                    {
                        ...
                        {
                            ip_write(cSocket, (char *)MDM_BUSY, strlen((char *)MDM_BUSY));
                        }
                        else
                        {
